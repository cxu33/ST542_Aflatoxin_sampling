...
Updated function that includes MLE fit
...
run_simulation_rand_param <- function(n_sim, 
  ...
  for (i in seq_along(candidate_sample_sizes)) {
    ...
    for (j in 1:n_sim) {
      ...
      gamma.fit <- tryCatch(fitdistr(x = sample_ppb[sample_ppb > 0], densfun = "gamma"), 
...
ggplot(thresholds.tb[thresholds.tb$sample_size <= 30,], aes(x = threshold, y = frac_accepted, color = train_safety_by_prop, fill = train_safety_by_prop)) +
  geom_point()+
...
```{r}
ggplot(thresholds.tb[thresholds.tb$threshold == 3,], 
       aes(x = sample_size,
           y = frac_accepted, 
...
I should check the code... but I have a feelign the issue that there is only so well that you can predict the maximum value of the train based on the sample average.
